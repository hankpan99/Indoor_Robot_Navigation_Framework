...
def RRT(img, img2, start, end, stepSize):
    ...
    while pathFound==False:
        ...
        if directCon and nodeCon:
            ...
            print(len(node_list[i].parent_x))
            ...
            for j in range(len(node_list[i].parent_x)):
                tmp_points.append(np.array([float(node_list[i].parent_x[j]), float(node_list[i].parent_y[j])]))
            tmp_points.append(np.array([float(end[0]), float(end[1])]))

            print('path points: ')
            for j in range(len(tmp_points)):
                tmp_points[j][0] = (-0.14 + (tmp_points[j][0] - 15.)*x_interval)*1000./255.*10.
                tmp_points[j][1] = (0.17 + (tmp_points[j][1] - 9.)*y_interval)*1000./255.*10.
                print(tmp_points[j][0], tmp_points[j][1])

            a = np.array(tmp_points)
            np.save(f'../data/navigate_path/{filename}/path.npy', a)

            # print()
            # print('distances between path points')
            # for j in range(len(tmp_points)-1):
            #     print(distance.euclidean(tmp_points[j], tmp_points[j+1]))
            
            # print()
            # print('angles between path vectors')
            for j in range(len(tmp_points)-2):
                vector_1 = tmp_points[j+1] - tmp_points[j]
                vector_2 = tmp_points[j+2] - tmp_points[j+1]
            ...
            cv2.imwrite(f"../data/map/{filename}/out.jpg",img2)
            break
